...
class HomeScreen(Screen):
    ...
    def save_personal_stats(self, req, result):
        ...
        if not self.PlungeApp.currencies:
            self.saving_personal_stats = False
            return
    ...
    def show_graph_popup(self, title):
        ...
        button_layout = BoxLayout(size_hint=(1, 0.1), spacing=50)
        ...
        padding = ((self.graph_popup.width - ((button.width + capture_button.width) + 50)) / 2)
    ...
    def show_pool_info(self):
        ...
        for d in data:
            grid.add_widget(Label(text=self.PlungeApp.get_string(d[0]) + ":", font_size=28,
                                  halign='right', markup=True))
            grid.add_widget(Label(text=str(d[1]), font_size=28, halign='left', markup=True))
    ...
    def show_exchange_info(self):
        ...
        for d in data:
            top_grid.add_widget(Label(text=str(d[0]), font_size=28, halign='right', markup=True))
            top_grid.add_widget(Label(text=str(d[1]), font_size=28, halign='left', markup=True))
        ...
        if self.primary_exchange in self.stats:
            if self.primary_currency in self.stats[self.primary_exchange]:
                ...
                for x in range(1, (len(stats['ask']['orders'])+1)):
                    ...
                    side_layout.add_widget(HeadingLabel(text=self.PlungeApp.get_string('Ask'), font_size=30))
                    side_layout.add_widget(HeadingLabel(text=self.PlungeApp.get_string('Bid'), font_size=30))
                    bottom_layout.add_widget(side_layout)
                    column_layout = BoxLayout(orientation='horizontal')
                    column_layout.add_widget(SubHeadingLabel(text=self.PlungeApp.get_string('Amount'), font_size=28))
                    column_layout.add_widget(SubHeadingLabel(text=self.PlungeApp.get_string('Rate'), font_size=28))
                    column_layout.add_widget(SubHeadingLabel(text=self.PlungeApp.get_string('Amount'), font_size=28))
                    column_layout.add_widget(SubHeadingLabel(text=self.PlungeApp.get_string('Rate'), font_size=28))
                    ...
                    for order in orders:
                        ...
                        order_grid.add_widget(Label(text='%.8f' % order[0]['amount'] if order[0]['amount'] > 0 else '%d' % order[0]['amount'], font_size=26))
                        order_grid.add_widget(Label(text='%.4f' % order[0]['cost'] if order[0]['cost'] > 0 else '%d' % order[0]['cost'], font_size=26))
                        order_grid.add_widget(Label(text='%.8f' % order[1]['amount'] if order[1]['amount'] > 0 else '%d' % order[1]['amount'], font_size=26))
                        order_grid.add_widget(Label(text='%.4f' % order[1]['cost'] if order[1]['cost'] > 0 else '%d' % order[1]['cost'], font_size=26))
        ...
        button_layout.add_widget(button)
        content.add_widget(button_layout)
        title = "%s %s ( ask / bid )" % (self.PlungeApp.get_string(self.primary_exchange),
                                         self.PlungeApp.get_string("Info"))
        self.info = Popup(title=title, content=content, auto_dismiss=False, size_hint=(0.9, 0.9))
        button.bind(on_press=self.close_info)
        self.info.open()
        padding = ((self.popup.width - button.width) / 2)
        button_layout.padding = (padding, 0, padding, 0)
